...
{
	name = "[TF2] Spawn Tools",
	author = "blood",
	description = "A simple system to change spawn systems in TF2",
	version = VERSION,
	url = ""
}
...
public Action:ShowEditModeGoodies(Handle:timer)
{
	...
	for (new i = MaxClients; i < maxEnt; i++)
	{
		...
		{
			if (StrEqual(sClassName, "info_player_teamspawn"))
			{
				tsCount++;
	...
	PrintHintTextToAll("TF2 Spawns: %i", tsCount);
...
RemoveAllDefaultSpawns()
{
	...
	{
		if (IsValidEdict(i) && IsValidEntity(i) && GetEdictClassname(i, sClassName, sizeof(sClassName)) &&
		(StrEqual(sClassName, "info_player_teamspawn")))
			RemoveEdict(i);
	}
}
