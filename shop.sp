...
public void OnPluginStart()
{
	...
	CreateTimer(60.0, GiveCreditTimer, _, TIMER_REPEAT);
...
public Action GiveCreditTimer(Handle timer)
{
	for (int i = 1; i <= MaxClients; i++)
	{
		if(IsClientInGame(i))
		{
			if (CheckCommandAccess(i, "", ADMFLAG_RESERVATION, true)) 
			{
				GiveCredits(i, 4, CREDITS_BY_BUY_OR_SELL);
				PrintToChat(i, "\x04[Shop] \x01You have been given 4 credits for playing on the server!");
			}
			else
			{
				GiveCredits(i, 2, CREDITS_BY_BUY_OR_SELL);
				PrintToChat(i, "\x04[Shop] \x01You have been given 2 credits for playing on the server!");
			}	
		}
	}
